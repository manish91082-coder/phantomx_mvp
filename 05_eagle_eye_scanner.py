...
# The Boss's Eagle Eye Scanner - Nano-Second Multi-Pair Engine
print("🦅 Booting PhantomX Eagle Eye Engine...")
...
discovered_pairs = {} # Symbol -> {"quickswap": address, "uniswapV3": address, "token": address}
...
async def discover_pools(session, rpc_url):
    ...
    for symbol in symbols:
        ...
        # UniswapV3 getPool (assuming 0.05% fee tier = 500)
...
async def scan_market(session, rpc_url):
    print("\n🚀 Initiating Infinite Eagle Eye Scan...")
    ...
    while True:
        ...
        for symbol in symbols:
            ...
            # Decode Quickswap Reserves
            qs_data = return_data[idx]
            qs_price = 0
            if len(qs_data) >= 96: # uint112, uint112, uint32 is 3 slots = 96 bytes
            ...
            # Decode UniswapV3 slot0
            uv3_data = return_data[idx+1]
            uv3_price = 0
            if len(uv3_data) >= 224: # 7 slots = 224 bytes
            ...
            color = "\033[92m" if spread_pct > 0.5 else "\033[90m" # Green if spread > 0.5%, else Gray
            reset = "\033[0m"
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        await scan_market(session, rpc_url)
